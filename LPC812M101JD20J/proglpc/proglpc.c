...
//-----------------------------------------------------------------------------
int read_data_word_prime ( unsigned int addr )
{
...
//printf("read 0x%08X\n",addr);
    ...
    while(1)
    {
        ...
        if(rb==8)
        {
            if(rdata[0]!=0x0A) return(1);
            if(rdata[1]!=0x30) return(1);
            if(rdata[2]!=0x0D) return(1);
            if(rdata[3]!=0x0A) return(1);
            ser_dump(rb);
            break;
        }
    }
...
int read_data_word ( unsigned int addr, unsigned int *data )
{
    ...
    while(1)
    {
        ...
        if(rb==7)
        {
            //for(ra=0;ra<rb;ra++) printf("0x%02X ",rdata[ra]); printf("\n");
            if(rdata[0]!=0x30) return(1);
            if(rdata[1]!=0x0D) return(1);
            if(rdata[2]!=0x0A) return(1);
            rc=0;
            rc<<=8; rc|=rdata[6];
            rc<<=8; rc|=rdata[5];
            rc<<=8; rc|=rdata[4];
...
int read_data_word_post ( void )
{
    ...
    while(1)
    {
        ...
        if(rb==3)
        {
            ...
            if(rdata[1]!=0x0D) return(1);
            if(rdata[2]!=0x0A) return(1);
            ser_dump(rb-1); //leave the 0x0A, fix some other day
            break;
        }
    }
    return(0);
...
{
    ramadd=0x00000000;
    if(read_data_word_prime(ramadd)) return(1);
    for(ra=0;ra<fdatalen;ra++)
    {
        if(read_data_word(ramadd,&rc)) return(1);
        if(rc!=fdata[ra])
        {
    ...
    if(read_data_word_post()) return(1);
}
    ...
    //read_data_word(0x10000000,&rc);
    //read_data_word(0x000002FC,&rc);
    //printf("read 0x%08X\n",rc);
    ...
    //printf("0x%08X\n",fdata[0]);
    rc=0;
    for(ra=0;ra<7;ra++) rc+=fdata[ra];
    fdata[ra]=-rc;

    romadd=0x00000000;
    //romsec=0;
    for(ra=0;ra<fdatalen;ra+=(0x400>>2))
    {
        romsec=romadd>>10; //LPC81X uses 1KB sectors
        printf("\nErasing 0x%04X\n",romadd);

        sprintf((char *)sdata,"P %u %u\r\n",romsec,romsec);
        ser_senddata(sdata,strlen((char *)sdata));
        if(get_return_code()) return(1);

        sprintf((char *)sdata,"E %u %u\r\n",romsec,romsec);
        ser_senddata(sdata,strlen((char *)sdata));
        if(get_return_code()) return(1);
        romadd+=0x400;
        //romsec++;
    ...
    for(ra=0;ra<fdatalen;ra+=16)
    {
        ...
        ramadd=0x10000300;
        rc=ramadd;
        for(rb=0;rb<16;rb++) //64 bytes per copy
        {
